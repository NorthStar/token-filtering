...
# approx 13M, 29M, 62M, 125M, 250M, 500M, 1B, 1.8B, n_embd/n_layer = 64
params = {
    'n_layer' : [2,   4,   7,   10,  14,  20,   26,   32],
    'n_head' :  [4,   4,   8,   10,  14,  10,   16,   16],
    'n_embd' :  [128, 256, 448, 640, 896, 1280, 1664, 2048]
}
...
template_config['wandb_project'] = 'medical-adamw'
template_config['out_dir'] = os.path.join(MODEL_PATH, 'adamw-scaling')
...
os.makedirs('adamw-scaling', exist_ok=True)
os.makedirs(os.path.join(MODEL_PATH, 'adamw-scaling'), exist_ok=True)
...
for i, total_params_val in enumerate(total_params):
    ...
    with open(f'adamw-scaling/gpt-{wandb_run_name}-mask.yaml', 'w') as f:
        yaml.dump(config, f, default_flow_style=False, indent=2)

    # create config with remove_tokens=True
    config_remove = config.copy()
    config_remove['wandb_run_name'] = f'remove-{wandb_run_name}'
    config_remove['remove_tokens'] = True
    config_remove['mask'] = True

    with open(f'adamw-scaling/gpt-{wandb_run_name}-remove.yaml', 'w') as f:
        yaml.dump(config_remove, f, default_flow_style=False, indent=2)
    
    # create config with mask=False
    config_nomask = config.copy()
    config_nomask['wandb_run_name'] = f'nomask-{wandb_run_name}'
    config_nomask['mask'] = False
    
    with open(f'adamw-scaling/gpt-{wandb_run_name}-nomask.yaml', 'w') as f:
        yaml.dump(config_nomask, f, default_flow_style=False, indent=2)
    
    config_document = config.copy()
    config_document['data_path'] = os.path.join(DATA_PATH, 'docfilter')
    
    config_document['wandb_run_name'] = f'document-{wandb_run_name}'
    config_document['mask'] = False
    config_document['remove_tokens'] = True
    
    with open(f'adamw-scaling/gpt-{wandb_run_name}-document.yaml', 'w') as f:
        yaml.dump(config_document, f, default_flow_style=False, indent=2)