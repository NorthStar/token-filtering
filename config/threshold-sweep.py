...
thresholds = list(thresholds['threshold'])
...
template_config['wandb_project'] = 'medical-threshold'
template_config['out_dir'] = os.path.join(MODEL_PATH, 'threshold')
...
os.makedirs(os.path.join(MODEL_PATH, 'threshold'), exist_ok=True)
...
for i, total_params_val in enumerate(total_params):
    ...
    for threshold in thresholds:
        ...
        config_threshold['mask_threshold'] = threshold
        config_threshold['wandb_run_name'] = f'mask-{wandb_run_name}-{int(threshold * 100)}'
        
        with open(f'threshold/gpt-{wandb_run_name}-{int(threshold * 100)}.yaml', 'w') as f:
